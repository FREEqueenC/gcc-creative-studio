...
"""
User Provisioning & Context Orchestration.

This module handles the business logic for setting up a fully authenticated user
context. It takes the verified session data (from session.py) and:
1. Ensures the user exists in the database (Just-In-Time provisioning).
2. Ensures the user belongs to an Organization (creating one if needed).
3. Ensures default Workspaces exist.
4. Resolves permissions and roles via OpenFGA.
"""
