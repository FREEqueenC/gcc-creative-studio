...
"""
Token Verification & Extraction.

This module is responsible for extracting the authentication token (Bearer) from
the request headers and verifying its validity against the appropriate provider
(Firebase Auth for local/dev, Google Identity Platform for production).

It returns the raw decoded token claims (session data) but does NOT interact
with the application database or perform user provisioning.
"""
